...
class TeachersFragment : Fragment(), SearchView.OnQueryTextListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val factory =
            UserViewModelFactory((context!!.applicationContext as BaseApplication).repository)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recycler_view_teachers.visibility = View.GONE

        Handler().postDelayed({
            progress_teacher.visibility = View.GONE
            recycler_view_teachers.visibility = View.VISIBLE
        }, 800)

        recycler_view_teachers.layoutManager = LinearLayoutManager(context)
        adapter = UserAdapter(R.layout.teacher_layout) { user ->
            onClick(user)
        }

        recycler_view_teachers.adapter = adapter

        userViewModel.allTeachers.observe(viewLifecycleOwner) { list ->
    ...
    private fun searchDatabase(query: String) {
        ...
        userViewModel.getSearchedTeachers(searchedQuery).observe(viewLifecycleOwner, { list ->
            adapter.submitList(list)
            if (list.isEmpty()) {
                nothing_layout.visibility = View.VISIBLE
            } else {
